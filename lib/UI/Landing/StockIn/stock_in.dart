...
import 'package:ramanas_waiter/UI/Landing/StockIn/Helper/stockIn_helper.dart';
import 'package:ramanas_waiter/UI/Landing/StockIn/widget/productModel.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:ramanas_waiter/Alertbox/snackBarAlert.dart';
import 'package:ramanas_waiter/Bloc/StockIn/stock_in_bloc.dart';
...
class StockView extends StatelessWidget {
  final from;
  const StockView({super.key, this.from});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => StockInBloc(),
      child: StockViewView(from: from),
    );
  }
}
...
class StockViewView extends StatefulWidget {
  final from;
  const StockViewView({super.key, this.from});
...
class StockViewViewState extends State<StockViewView> {
  ...
  void initState() {
    super.initState();
    context.read<StockInBloc>().add(StockDetails());
    context.read<StockInBloc>().add(StockInLocation());
    setState(() {
      stockLoad = true;
    });
  ...
  Widget build(BuildContext context) {
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                ...
                child: TextFormField(
                  key: ValueKey(
                    'tax1Amount_${index}_${productRow.tax1Amount}',
              ...
              SizedBox(
                ...
                child: TextFormField(
                  key: ValueKey(
                    'total_${index}_${productRow.total}',
                  ), // Fixed: removed underscore
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: (value) {
                  setState(() {
                    try {
                      selectedProductObj = getAddProductModel.data?.firstWhere(
                        ...
                        onPressed: () async {
                          ...
                          try {
                            ...
                            context.read<StockInBloc>().add(
                              SaveStockIn(jsonEncode(payload)),
                            );
                          } catch (e) {
                            setState(() {
                              saveLoad = false;
    ...
    return Scaffold(
      ...
      body: BlocBuilder<StockInBloc, dynamic>(
        buildWhen: ((previous, current) {
          ...
          if (current is GetLocationModel) {
            ...
            if (getLocationModel.success == true) {
              ...
              context.read<StockInBloc>().add(
                StockInSupplier(locationId.toString()),
              );
              context.read<StockInBloc>().add(
                StockInAddProduct(locationId.toString()),
              );
              setState(() {
                stockLoad = false;
              });
          ...
          if (current is GetSupplierLocationModel) {
            ...
            if (getSupplierLocationModel.errorResponse?.isUnauthorized ==
                true) {
              _handle401Error();
              return true;
            }
